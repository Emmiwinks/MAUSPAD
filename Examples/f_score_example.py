...
ticker = 'WM'
raw_IS = fd.get_income_statement_quarterly(ticker)
income_statement = raw_IS[0].T[2:]
income_statement.columns = list(raw_IS[0].T.iloc[0])


raw_BS = fd.get_balance_sheet_quarterly(ticker)
balance_sheet = raw_BS[0].T[2:]
balance_sheet.columns = list(raw_BS[0].T.iloc[0])

raw_CS = fd.get_cash_flow_quarterly(ticker)
cash_flow = raw_CS[0].T[2:]
cash_flow.columns = list(raw_CS[0].T.iloc[0])

# income_statement.to_csv('income_statement_quarterly.csv')
# balance_sheet.to_csv('balance_sheet_quarterly.csv')
# cash_flow.to_csv('cash_flow_quarterly.csv')
# fd.get_balance_sheet_quarterly(ticker).to_csv('balance_sheet_quarterly.csv')
# fd.get_cash_flow_quarterly(ticker).to_csv('cash_flow_quarterly.csv')
# fd.get_income_statement_quarterly(ticker).to_csv('income_statement_quarterly.csv')
